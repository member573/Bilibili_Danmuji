...
1. 自动记录盲盒开盒成本、爆出回报和盈亏（按房间聚合）。
2. 回报价值优先按 `gift_tip_price` 计算（表示实际爆出礼物价值），`original_gift_price` 视为开盒成本相关字段，不参与回报计算。
3. 支持弹幕命令查询：
   - `近3月盲盒`：查询近3个月汇总
   - `4月盲盒`（`1-12月盲盒`）：查询指定月份汇总
4. 前端盲盒统计面板仅展示“本房间全体盲盒汇总（主播视角）”，单位为“金仓鼠”，文案字段为“成本 / 回报 / 盈亏 / 盲盒数”。
5. 开发调试模式（`test_mode=true`）下，发送弹幕仅打印日志不会实际发到直播间。
